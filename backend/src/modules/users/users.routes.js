...
const userUpdateSchema = {
  ...
  password: [validators.optional(validators.password("Password"))],
  role: [validators.optional(validators.oneOf("Role", userRoles))]
};

usersRouter.get("/", authorize(["admin", "hr"]), usersListHandler);
usersRouter.post("/", authorizeRolesOnly(["admin"]), validateBody(userCreateSchema), createUserHandler);
usersRouter.get("/summary", authorize(["admin", "hr"]), usersSummaryHandler);
usersRouter.get("/module-catalog", authorizeRolesOnly(["admin"]), moduleCatalogHandler);
usersRouter.put("/:id/module-access", authorizeRolesOnly(["admin"]), userIdParam, updateModuleAccessHandler);
...
usersRouter.put("/:id", authorizeRolesOnly(["admin"]), userIdParam, validateBody(userUpdateSchema), updateUserHandler);
usersRouter.delete("/:id", authorizeRolesOnly(["admin"]), userIdParam, deleteUserHandler);
